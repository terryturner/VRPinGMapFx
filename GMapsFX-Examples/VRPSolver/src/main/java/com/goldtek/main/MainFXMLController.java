...
public class MainFXMLController implements Initializable, MapComponentInitializedListener, DirectionsServiceCallback {
	...
	@FXML
	protected ListView<ColorfulDepot> RouteGuide;
	@FXML
	protected MenuButton MenuButton;
	@FXML
	protected CheckMenuItem MenuCostAssist;
	...
	public void directionsReceived(DirectionsResult results, DirectionStatus status) {
		if (status.equals(DirectionStatus.OVER_QUERY_LIMIT)) {
			final Runnable update = new Runnable() {
				@Override
				public void run() {
					mLostRoute++;
					drawDriections(StartEndPoint.get(mLostRoute), StartEndPoint.get(mLostRoute),
							RoutePoint.get(mLostRoute));
					mDelayFactor++;
				}
			};
			...
			mDelayFactor++;
			mtimer = new Timer(mDelayFactor * 1250, delayedAction);
	...
	private void afterSolve(IVrpSolver solver, List<Route> routes) {
		if (routes != null) {
			for (Route route : routes) {
				...
				mMenuButtonText.add(String.format("%s - %s", RouteLabel.getInstance().get(index),
						RouteLabel.getInstance().getDriver(index)));
	...
	private void drawDriections(Depot start, Depot end, Route route) {
		...
		markerOptions.position(new LatLong(start.getLatitude(), start.getLongitude()))
				.icon("http://maps.google.com/mapfiles/kml/pal3/icon21.png").label("M");
		Marker marker = new Marker(markerOptions);
		MapView.getMap().addMarker(marker);

		DirectionsRenderer directionsRenderer = new DirectionsRenderer(false, MapView.getMap(), mDirectionsPane,
			...
			if (i == index) {
				...
				if (index <= 9) {
					if (mGMapLineList.get(i).getVisible() == true) {
						...
						List<Marker> marker = mGMapLineList.get(i).getMarker();
						for (Marker markers : marker) {
							MapView.getMap().removeMarker(markers); // hide
																	// markers
						}
				...
				if (((MenuItem) event.getSource()).getText().equals(TITLE_ALL_ROUTE)) {
					for (GMapLine lines : mGMapLineList) { // Show Total Routes
						if (lines.getVisible() == false) {
							...
							List<Marker> marker = lines.getMarker();
							for (Marker markers : marker) {
								MapView.getMap().addMarker(markers); // show
																		// markers
							}
							lines.setVisible(true);
						}
					...
					MenuButton_ShowRoute(idx); // menu button action event
												// functions
